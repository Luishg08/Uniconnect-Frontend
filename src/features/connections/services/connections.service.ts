...
import { AcceptRejectResponse, ConnectionRequest, ConnectionResponse, SendConnectionRequestDto } from '../types';

class ConnectionService {
    async getPendingRequests(): Promise<ConnectionRequest[]> {
        const response = await api.get<ConnectionRequest[]>(CONNECTION_ENDPOINTS.PENDING_REQUESTS);
        return response.data;
    }

    async sendConnectionRequest(data: SendConnectionRequestDto): Promise<ConnectionResponse> {
        const response = await api.post<ConnectionResponse>(CONNECTION_ENDPOINTS.SEND_REQUEST, data);
        return response.data;
    }
